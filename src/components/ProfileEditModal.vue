      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="mb-6 flex flex-col items-center">
          <div class="relative group mb-2">
            ...
            <!-- 上传按钮覆盖 -->
            <div class="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/40 rounded-full transition-all duration-300">
              <label class="opacity-0 group-hover:opacity-100 cursor-pointer text-white bg-primary-light px-2 py-1 rounded-lg text-sm">
                更换头像
                <input 
                  type="file" 
                  accept="image/*" 
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // 提交更新
    const success = await profileStore.updateUserProfile(props.userId, updateData)

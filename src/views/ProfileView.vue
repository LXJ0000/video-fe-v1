    ...
    <template v-else-if="profileStore.userProfile">
        ...
        <div class="md:flex">
          ...
          <div class="p-8 text-center md:text-left md:flex md:items-center">
            ...
            <div class="flex-shrink-0 mx-auto md:mx-0 mb-4 md:mb-0 md:mr-6">
              <div class="w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden bg-gray-200 dark:bg-gray-700">
                <img 
            ...
            <div>
              <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
                {{ profileStore.userProfile.nickname || profileStore.userProfile.username }}
              </h1>
              <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                注册于 {{ formatDate(profileStore.userProfile.createdAt) }}
              </p>
              <p v-if="profileStore.userProfile.bio" class="mt-2 text-gray-600 dark:text-gray-300">
                {{ profileStore.userProfile.bio }}
              </p>
              <p v-else class="mt-2 text-gray-500 dark:text-gray-400 italic text-sm">
                这个人很懒，还没有填写个人简介...
              </p>
          ...
          <!-- 右侧统计信息 -->
          <div class="border-t md:border-t-0 md:border-l border-gray-200 dark:border-gray-700 p-8 bg-gray-50 dark:bg-gray-800/50">
            <h2 class="text-lg font-medium text-gray-900 dark:text-white mb-4">数据统计</h2>
            <div class="grid grid-cols-3 gap-4">
              <div class="text-center">
                <div class="text-2xl font-bold text-primary-dark dark:text-primary-light">
      ...
      <div class="min-h-[400px]">
        ...
        <div v-if="activeTab === 'videos'" class="space-y-6">
          <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-6">上传的视频</h2>
          ...
          <div v-else class="text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
            ...
            <p class="mt-1 text-gray-500 dark:text-gray-400">该用户还没有上传任何视频</p>
          </div>

          <!-- 加载更多按钮 -->
          <div v-if="hasMoreVideos && !isLoadingVideos" class="text-center mt-8">
            <button 
        ...
        <div v-else-if="activeTab === 'history'" class="space-y-6">
          <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-6">观看历史</h2>
          
          <!-- 加载历史 -->
          <div v-if="profileStore.isLoadingHistory" class="flex justify-center py-8">
            <div class="animate-spin rounded-full h-8 w-8 border-2 border-primary-light border-t-transparent"></div>
          </div>
          ...
          <div v-else class="text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">暂无观看记录</h3>
            <p class="mt-1 text-gray-500 dark:text-gray-400">去发现一些精彩视频吧</p>
          </div>

          <!-- 加载更多按钮 -->
          <div v-if="profileStore.historyHasMore && !profileStore.isLoadingHistory" class="text-center mt-8">
            <button 
        ...
        <div v-else-if="activeTab === 'favorites'" class="space-y-6">
          <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-6">收藏的视频</h2>
          
          <!-- 加载收藏 -->
          <div v-if="profileStore.isLoadingFavorites" class="flex justify-center py-8">
            <div class="animate-spin rounded-full h-8 w-8 border-2 border-primary-light border-t-transparent"></div>
          </div>
          ...
          <div v-else class="text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-xl">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white">暂无收藏</h3>
            <p class="mt-1 text-gray-500 dark:text-gray-400">收藏喜欢的视频，方便以后观看</p>
          </div>

          <!-- 加载更多按钮 -->
          <div v-if="profileStore.favoritesHasMore && !profileStore.isLoadingFavorites" class="text-center mt-8">
            <button 
...
const loadMoreHistory = () => {
  profileStore.fetchWatchHistory(userId.value, profileStore.historyCurrentPage + 1)
}

// 加载更多收藏
const loadMoreFavorites = () => {
  profileStore.fetchFavorites(userId.value, profileStore.favoritesCurrentPage + 1)
}

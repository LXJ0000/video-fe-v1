<template>
  <div class="video-manage-view container mx-auto px-4 py-8">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
      <div class="text-2xl font-semibold bg-gradient-to-r from-primary-light via-primary to-primary-dark bg-clip-text text-transparent">
        视频管理
      </div>
      <div class="flex items-center gap-4">
        ...
        >
          ...
          批量删除 {{ selectedVideos.length ? `(${selectedVideos.length})` : '' }}
        </button>
      </div>
    </div>
    ...
    <div v-if="isLoading" class="py-16 text-center">
      <div class="inline-block animate-spin rounded-full h-8 w-8 border-[3px] border-[#FF1493] border-t-transparent"></div>
    </div>
...
import type { VideoItem, VideoStatusFilter, SortField, SortOrder } from '../types/video'
...
const isAllSelected = computed(() => {
  return videoStore.videos.length > 0 && selectedVideos.value.length === videoStore.videos.length
})

const isIndeterminate = computed(() => {
  return selectedVideos.value.length > 0 && selectedVideos.value.length < videoStore.videos.length
})
...
const confirmBatchDelete = async () => {
  try {
    ...
    videoStore.videos = videoStore.videos.filter(v => !selectedVideos.value.includes(v.id))
    selectedVideos.value = []
